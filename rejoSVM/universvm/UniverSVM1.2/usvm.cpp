...
const char *kernel_type_table[] = {"linear","polynomial","rbf","sigmoid","custom"};
const char *optimizer_table[] = {"svqp2","cccp"};
...
void setup_standard_svm_problem(SVQP2* sv){
	...
	if (Y[data_map[TRAIN][j]] <0){
	    ...
	    sv->cmin[fill_level]=-weights[fill_level]; sv->cmax[fill_level]=0; // rsousa
	}else{
	    sv->b[fill_level]= 1;
	    //sv->cmin[fill_level]=0; sv->cmax[fill_level]=C;
	    sv->cmin[fill_level]=0; sv->cmax[fill_level]=weights[fill_level];  // rsousa
	}
	fill_level++;
    }
		 ...
		 )
{
  ...
  //if (nrhs!=3)  
  if (nrhs!=4) //rsousa
    mexErrMsgTxt("4 input arguments needed.\n");
