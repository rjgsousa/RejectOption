...
{
	mexPrintf(
	"Usage: model = svmtrain(training_label_vector, training_instance_matrix, 'libsvm_options', weights);\n"
	"libsvm_options:\n"
...
int read_problem_dense(const mxArray *label_vec, const mxArray *instance_mat, const mxArray *weights_vec)
{
	int i, j, k;
	...
	weights = mxGetPr(weights_vec); // mine
	...
	prob.w = Malloc(double,prob.l); // rsousa
	...
	if ( nrhs == 4 ) // rsousa
	{
		...
		if(mxIsSparse(prhs[1]))
		{
			if(param.kernel_type == PRECOMPUTED)
			{
				...
				err = read_problem_dense(prhs[0], lhs[0],prhs[3]); // rsousa
		  ...
		  err = read_problem_dense( prhs[0], prhs[1], prhs[3] ); // rsousa
		// err = read_problem_dense(prhs[0], prhs[1] ); // libsvm original code
		...
		if(cross_validation)
		{
			...
			ptr = mxGetPr(plhs[0]);
			//ptr[0] = do_cross_validation(); // commented by rsousa
			mexPrintf("Error: Not Implemented\n");
			return;
		}
		else
		{
