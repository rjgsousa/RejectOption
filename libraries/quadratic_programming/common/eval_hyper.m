function d = eval_hyper(hyper, d)
% used to set parameters -- needed for initializing objects 
% 
...
for j=1:length(hyper)
  if iscell(hyper{j}) hyper{j}=hyper{j}{1}; 
...
if length(hyper)==1 & (isa(hyper,'kernel'))
      a.child=hyper;
else
                                               % evaluate input

 for i=1:size(hyper,2)
      if isa(hyper{i},'kernel')
	a.child=hyper{i};
      else
           value=hyper{i};
           if ischar(value)
